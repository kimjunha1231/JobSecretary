...
- 승인된 이력서·포트폴리오·기존 자기소개서 카드에서 `활동 후보 만들기`를 실행하면 서버가 Gemini에 원문 fragment를 불신 데이터로 전달하고, 프로젝트·경력·교육·자격·어학·수상 등의 후보를 구조화합니다.
...
- `/career`의 승인된 활동 영역에서 제목·조직·역할·요약·기술·성과·지표를 검색하고 프로젝트·경력·교육·자격·어학·수상·리더십 등 종류별로 좁힐 수 있습니다.
- 활동 라이브러리에서 직접 활동을 등록할 수 있으며, 작성 작업대의 직접 추가도 같은 입력 폼을 사용합니다. 자격·어학은 발급/시험 기관·취득/응시 시점·점수/등급을 별도 항목으로 입력할 수 있습니다.
- 원본 fragment가 연결되지 않은 활동은 `/career`에서 직접 수정할 수 있습니다. 원본 인용이 연결된 활동은 인용과 사실이 어긋나지 않도록 수정 버튼을 제공하지 않습니다.
- 승인 활동은 삭제 대신 보관할 수 있고 보관함에서 복원할 수 있습니다. 보관된 활동·근거는 작성 context와 PDF 출력 대상에서 제외되며, 원본 인용과 활동 내용은 보존됩니다.
...
- HTML은 브라우저에 렌더링하지 않고 `script`·`style`·주석 등 실행·장식 블록을 제거한 텍스트와 heading/문단 fragment만 저장합니다. 공개 PDF URL은 기존 PDF 텍스트 추출기로 연결해 페이지 fragment와 원본 바이너리를 함께 보존합니다. 최종 URL과 수집 시각도 함께 보존합니다.
...
- 초안 비교 화면에서는 문단마다 후보를 직접 하나씩 선택해야 하나의 편집 초안을 만들 수 있으며, 병합 결과는 사용자 revision으로 남습니다. 후보 첫 문단을 자동으로 채우지 않아 사용자의 선택 없이 결과가 확정되지 않습니다.
...
- 저장된 프로필에서도 문장 길이(최소·최대·평균)·문장 끝맺음·선호 연결어·금칙어·과장 정도를 직접 수정할 수 있습니다. 최소값이 최대값보다 큰 설정은 서버에서 거부하며, 과장 정도를 `기본`으로 되돌릴 때는 기존 값을 null로 초기화합니다. 분석 결과를 그대로 덮어쓰지 않고 사용자가 저장 버튼을 눌러야 다음 생성에 반영됩니다.
...
- 서버는 선택한 ID가 현재 사용자의 승인 예문인지 다시 확인하고, 세션 조회·생성 context에도 같은 선택을 적용합니다. 직접 고른 문항별 예문은 자동 범위(현재 문항·전역)를 넘어도 같은 사용자 프로필의 말투 자료로 반영하며, 현재 문항 예문을 먼저 배치합니다. 말투 프로필을 바꾸거나 선택을 초기화하면 새 세션에서 다시 결정할 수 있습니다.
...
- `/career`의 `승인된 활동 근거` 영역에서 프로젝트·경력·교육·자격·어학·수상 활동을 조직/역할·기여·행동·결과·성과·기술 단위로 확인할 수 있습니다. 이 목록과 PDF 모두 `approved` 상태만 사용합니다.
...
- 2026-09-21에는 활동·자료 검색 필터와 명시적 문단·활동 선택을 포함한 최신 로컬 커밋을 Vercel Preview(`coverlettervault-8pu2lhy4l-junhas-projects-a748ef77.vercel.app`)로 배포해 `READY` 상태를 확인했습니다. 보호를 우회한 `vercel curl` 기준 `/` 200, 비로그인 `/career` 307, 승인 활동 API 401을 확인했으며 `vercel logs --level error --level warning`에서 조회 시점 로그가 없었습니다. Production alias와 환경변수는 변경하지 않았습니다.
