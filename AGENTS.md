...
3. **모든 변경 사항은 단 한 번의 지시로 설계, 가상 환경 개발, 자체 리뷰, 병합까지 전 자동화(One-Shot)되어야 한다.**
...
3. 작업 완료 후 `npm run harness:verify` 기계적 검증과 `04_feedback_loops.md`의 시니어 리뷰를 스스로 통과하면, 알아서 `main`에 병합하고 가상 브랜치를 파기하세요.
