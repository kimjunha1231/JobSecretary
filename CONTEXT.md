...
- **Career Item / 경력 항목**: 프로젝트, 경력, 교육, 자격·어학, 수상, 리더십 등 재사용하는 상위 경험 단위.
- **Credential / 자격·어학 항목**: 취득·응시 사실이나 점수·등급을 원문으로 확인할 수 있는 자격증, 면허, 어학 시험 이력. 단순 교육 수료는 교육, 상훈·입상은 수상 이력으로 구분한다.
- **Evidence Record / 근거 기록**: 상황·문제·행동·결과·학습·수치를 묶은 검증 가능한 사실 단위.
- **User-authored Evidence Record / 사용자 직접 입력 근거**: 사용자가 직접 작성해 저장한 경력 사실. 자료에서 추출한 기록과 달리 원본 구간 없이도 사용자가 입력한 내용을 근거로 삼는다.
...
3. 자료에서 추출한 날짜·수치·고유명사·역할은 승인된 원본 구간으로 뒷받침한다. 사용자가 직접 작성한 사실은 명시적 저장 동작으로 확정된 사용자 직접 입력 근거를 출처로 삼는다.
