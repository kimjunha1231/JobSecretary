...
function profileDraftFrom(profile: Profile): ProfileDraft {
    return {
        name: profile.name,
        endingStyle: profile.endingStyle.join(', '),
        preferredConnectors: profile.preferredConnectors.join(', '),
        bannedExpressions: profile.bannedExpressions.join(', '),
        exaggerationLevel: typeof profile.exaggerationLevel === 'number' ? String(profile.exaggerationLevel) : '',
        sentenceMin: typeof profile.sentenceLength?.min === 'number' ? String(profile.sentenceLength.min) : '',
        sentenceMax: typeof profile.sentenceLength?.max === 'number' ? String(profile.sentenceLength.max) : '',
        sentenceAverage: typeof profile.sentenceLength?.average === 'number' ? String(profile.sentenceLength.average) : '',
    };
}
...
export function StyleProfileBoard() {
    ...
    const [form, setForm] = useState({ name: '', endingStyle: '', preferredConnectors: '', bannedExpressions: '', exaggerationLevel: '', sentenceMin: '', sentenceMax: '', sentenceAverage: '', example: '' });
    ...
    const loadProfiles = async () => {
        ...
        try {
            ...
            const nextProfiles = Array.isArray(result) ? result as unknown as ProfileResponse[] : [];
            setProfiles(nextProfiles);
            setProfileDrafts(current => nextProfiles.reduce<Record<string, ProfileDraft>>((next, item) => {
                next[item.profile.id] = current[item.profile.id] ?? profileDraftFrom(item.profile);
                return next;
            }, {}));
        ...
        try {
            const response = await fetch('/api/style-profiles', {
                ...
                body: JSON.stringify({
                    ...
                    exaggerationLevel: form.exaggerationLevel === '' ? undefined : Number(form.exaggerationLevel),
                    sentenceLength: {
                        min: optionalNumber(form.sentenceMin),
                        max: optionalNumber(form.sentenceMax),
                        average: optionalNumber(form.sentenceAverage),
                    },
                }),
            });
            ...
            setForm({ name: '', endingStyle: '', preferredConnectors: '', bannedExpressions: '', exaggerationLevel: '', sentenceMin: '', sentenceMax: '', sentenceAverage: '', example: '' });
            await loadProfiles();
        ...
        try {
            const response = await fetch(`/api/style-profiles/${profileId}`, {
                ...
                body: JSON.stringify({
                    ...
                    sentenceLength: {
                        min: optionalNumber(draft.sentenceMin),
                        max: optionalNumber(draft.sentenceMax),
                        average: optionalNumber(draft.sentenceAverage),
                    },
                }),
            });
            const result = await readJson(response);
            if (!response.ok) throw new Error(typeof result.error === 'string' ? result.error : '말투 프로필을 저장하지 못했습니다.');
            await loadProfiles();
            toast.success('말투 프로필 설정을 저장했습니다.');
        } catch (updateError) {
            toast.error(updateError instanceof Error ? updateError.message : '말투 프로필을 저장하지 못했습니다.');
        } finally {
            setProfileBusy(null);
        }
    };
        ...
        try {
            ...
            setProfileDrafts(current => {
                ...
                return draft ? {
                    ...
                    [profileId]: {
                        ...draft,
                        sentenceMin: typeof result.sentenceLength.min === 'number' ? String(result.sentenceLength.min) : '',
                        sentenceMax: typeof result.sentenceLength.max === 'number' ? String(result.sentenceLength.max) : '',
                        sentenceAverage: typeof result.sentenceLength.average === 'number' ? String(result.sentenceLength.average) : '',
                        endingStyle: result.endingStyle.join(', '),
                        preferredConnectors: result.preferredConnectors.join(', '),
                    },
                } : current;
            });
            await loadProfiles();
    ...
    return <div className="mx-auto max-w-5xl space-y-6 pb-20">
        ...
        <form onSubmit={createProfile} className="space-y-4 rounded-2xl border border-primary/20 bg-primary/5 p-5 md:p-6">
            ...
            <div className="grid gap-3 md:grid-cols-4"><label className="space-y-1.5 text-xs text-zinc-300"><span>문장 끝맺음</span><input value={form.endingStyle} onChange={event => setForm(current => ({ ...current, endingStyle: event.target.value }))} placeholder="했습니다, 배웠습니다" className="w-full rounded-lg border border-white/10 bg-background px-3 py-2.5 text-sm text-white outline-none focus:border-primary/60" /></label><label className="space-y-1.5 text-xs text-zinc-300"><span>선호 연결어</span><input value={form.preferredConnectors} onChange={event => setForm(current => ({ ...current, preferredConnectors: event.target.value }))} placeholder="먼저, 이후, 그래서" className="w-full rounded-lg border border-white/10 bg-background px-3 py-2.5 text-sm text-white outline-none focus:border-primary/60" /></label><label className="space-y-1.5 text-xs text-zinc-300"><span>피하고 싶은 표현</span><input value={form.bannedExpressions} onChange={event => setForm(current => ({ ...current, bannedExpressions: event.target.value }))} placeholder="열정적으로, 혁신적인" className="w-full rounded-lg border border-white/10 bg-background px-3 py-2.5 text-sm text-white outline-none focus:border-primary/60" /></label><label className="space-y-1.5 text-xs text-zinc-300"><span>과장 정도</span><select value={form.exaggerationLevel} onChange={event => setForm(current => ({ ...current, exaggerationLevel: event.target.value }))} className="w-full rounded-lg border border-white/10 bg-background px-3 py-2.5 text-sm text-white outline-none focus:border-primary/60"><option value="">기본</option><option value="0">담백하게</option><option value="0.25">약간 절제</option><option value="0.5">균형 있게</option><option value="0.75">강조해서</option><option value="1">매우 강조</option></select></label></div>
            <div><p className="text-xs text-zinc-400">문장 길이(선택)</p><div className="mt-2 grid gap-3 sm:grid-cols-3"><label className="space-y-1.5 text-xs text-zinc-300"><span>최소 글자 수</span><input type="number" min="1" max="10000" step="1" value={form.sentenceMin} onChange={event => setForm(current => ({ ...current, sentenceMin: event.target.value }))} placeholder="예: 30" className="w-full rounded-lg border border-white/10 bg-background px-3 py-2.5 text-sm text-white outline-none focus:border-primary/60" /></label><label className="space-y-1.5 text-xs text-zinc-300"><span>최대 글자 수</span><input type="number" min="1" max="10000" step="1" value={form.sentenceMax} onChange={event => setForm(current => ({ ...current, sentenceMax: event.target.value }))} placeholder="예: 90" className="w-full rounded-lg border border-white/10 bg-background px-3 py-2.5 text-sm text-white outline-none focus:border-primary/60" /></label><label className="space-y-1.5 text-xs text-zinc-300"><span>목표 평균 글자 수</span><input type="number" min="0" max="10000" step="1" value={form.sentenceAverage} onChange={event => setForm(current => ({ ...current, sentenceAverage: event.target.value }))} placeholder="예: 60" className="w-full rounded-lg border border-white/10 bg-background px-3 py-2.5 text-sm text-white outline-none focus:border-primary/60" /></label></div></div>
        ...
        {retrievalSummary && retrievalSummary.sessionCount > 0 && <section className="rounded-2xl border border-sky-300/20 bg-sky-300/5 p-5 md:p-6" aria-labelledby="retrieval-summary-title">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                    ...
                    <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">최근 {retrievalSummary.sessionCount}개 세션에서 사용자가 저장한 정답 활동 라벨을 우선 사용하고, 라벨이 없는 문항은 선택·고정 근거를 기준으로 계산한 기준선입니다. 자동 정답이나 모델 학습 데이터로 사용하지 않습니다.</p>
                </div>
        ...
        <section className="space-y-3" aria-labelledby="profile-list-title">
            ...
            {isLoading ? <div className="rounded-2xl border border-white/10 bg-surface/40 px-5 py-12 text-center text-sm text-zinc-500">말투 프로필을 불러오는 중입니다…</div> : profiles.length === 0 ? <div className="rounded-2xl border border-dashed border-white/15 bg-surface/30 px-5 py-12 text-center text-sm text-zinc-500">아직 말투 프로필이 없습니다.</div> : <div className="grid gap-4 md:grid-cols-2">{profiles.map(item => {
                ...
                const profileDraft = profileDrafts[item.profile.id] ?? profileDraftFrom(item.profile);
                return <article key={item.profile.id} className="rounded-2xl border border-white/10 bg-surface/50 p-5">
                    ...
                    <div className="mt-4 rounded-xl border border-white/10 bg-background/30 p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-medium text-zinc-200">프로필 설정 직접 조정</p><p className="mt-1 text-[11px] leading-5 text-zinc-500">분석 결과와 별개로 다음 생성에 적용할 표현을 직접 선택할 수 있습니다.</p></div><button type="button" onClick={() => void updateProfile(item.profile.id)} disabled={profileBusy !== null} className="inline-flex shrink-0 items-center justify-center gap-1.5 rounded-lg bg-primary/15 px-2.5 py-1.5 text-xs font-semibold text-primary transition hover:bg-primary/25 disabled:cursor-not-allowed disabled:opacity-50">{profileBusy === item.profile.id && <Loader2 size={13} className="animate-spin" aria-hidden="true" />} 설정 저장</button></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><label className="space-y-1 text-[11px] text-zinc-400"><span>프로필 이름</span><input value={profileDraft.name} onChange={event => setProfileDrafts(current => ({ ...current, [item.profile.id]: { ...profileDraft, name: event.target.value } }))} maxLength={100} className="w-full rounded-lg border border-white/10 bg-background px-2.5 py-2 text-xs text-white outline-none focus:border-primary/60" /></label><label className="space-y-1 text-[11px] text-zinc-400"><span>과장 정도</span><select aria-label={`${item.profile.name} 과장 정도`} value={profileDraft.exaggerationLevel} onChange={event => setProfileDrafts(current => ({ ...current, [item.profile.id]: { ...profileDraft, exaggerationLevel: event.target.value } }))} className="w-full rounded-lg border border-white/10 bg-background px-2.5 py-2 text-xs text-white outline-none focus:border-primary/60"><option value="">기본</option><option value="0">담백하게</option><option value="0.25">약간 절제</option><option value="0.5">균형 있게</option><option value="0.75">강조해서</option><option value="1">매우 강조</option></select></label><label className="space-y-1 text-[11px] text-zinc-400"><span>문장 끝맺음</span><input value={profileDraft.endingStyle} onChange={event => setProfileDrafts(current => ({ ...current, [item.profile.id]: { ...profileDraft, endingStyle: event.target.value } }))} placeholder="했습니다, 배웠습니다" className="w-full rounded-lg border border-white/10 bg-background px-2.5 py-2 text-xs text-white outline-none focus:border-primary/60" /></label><label className="space-y-1 text-[11px] text-zinc-400"><span>선호 연결어</span><input value={profileDraft.preferredConnectors} onChange={event => setProfileDrafts(current => ({ ...current, [item.profile.id]: { ...profileDraft, preferredConnectors: event.target.value } }))} placeholder="먼저, 이후, 그래서" className="w-full rounded-lg border border-white/10 bg-background px-2.5 py-2 text-xs text-white outline-none focus:border-primary/60" /></label></div><div className="mt-3"><p className="text-[11px] text-zinc-400">문장 길이(선택)</p><div className="mt-2 grid gap-2 sm:grid-cols-3"><label className="space-y-1 text-[11px] text-zinc-400"><span>최소 글자 수</span><input aria-label={`${item.profile.name} 최소 문장 길이`} type="number" min="1" max="10000" step="1" value={profileDraft.sentenceMin} onChange={event => setProfileDrafts(current => ({ ...current, [item.profile.id]: { ...profileDraft, sentenceMin: event.target.value } }))} className="w-full rounded-lg border border-white/10 bg-background px-2.5 py-2 text-xs text-white outline-none focus:border-primary/60" /></label><label className="space-y-1 text-[11px] text-zinc-400"><span>최대 글자 수</span><input aria-label={`${item.profile.name} 최대 문장 길이`} type="number" min="1" max="10000" step="1" value={profileDraft.sentenceMax} onChange={event => setProfileDrafts(current => ({ ...current, [item.profile.id]: { ...profileDraft, sentenceMax: event.target.value } }))} className="w-full rounded-lg border border-white/10 bg-background px-2.5 py-2 text-xs text-white outline-none focus:border-primary/60" /></label><label className="space-y-1 text-[11px] text-zinc-400"><span>목표 평균 글자 수</span><input aria-label={`${item.profile.name} 평균 문장 길이`} type="number" min="0" max="10000" step="1" value={profileDraft.sentenceAverage} onChange={event => setProfileDrafts(current => ({ ...current, [item.profile.id]: { ...profileDraft, sentenceAverage: event.target.value } }))} className="w-full rounded-lg border border-white/10 bg-background px-2.5 py-2 text-xs text-white outline-none focus:border-primary/60" /></label></div></div><label className="mt-2 block space-y-1 text-[11px] text-zinc-400"><span>피하고 싶은 표현</span><input value={profileDraft.bannedExpressions} onChange={event => setProfileDrafts(current => ({ ...current, [item.profile.id]: { ...profileDraft, bannedExpressions: event.target.value } }))} placeholder="열정적으로, 혁신적인" className="w-full rounded-lg border border-white/10 bg-background px-2.5 py-2 text-xs text-white outline-none focus:border-primary/60" /></label></div>
