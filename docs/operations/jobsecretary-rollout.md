...
- 2026-09-21 최신 로컬 커밋 Preview(`coverlettervault-16lwekclx-junhas-projects-a748ef77.vercel.app`)가 `READY`가 되었고, 보호를 우회하지 않은 `vercel curl`로 `/` 200, 비로그인 `/career`·`/style` 307, 승인 활동 API·검색 품질 집계 API·회원 탈퇴 `DELETE` 401을 확인했다. 활동 검색과 공개 PDF URL 수집, 레거시 계정 정리 변경을 포함한 Preview 로그에는 조회 시점 오류가 없었고 Production alias는 변경하지 않았다.
