...
- 2026-09-21 최신 로컬 커밋 Preview(`coverlettervault-i0ctvkh0s-junhas-projects-a748ef77.vercel.app`)가 `READY`가 되었고, 보호를 우회하지 않은 `vercel curl`로 `/` 200, 비로그인 `/style` 307, 새 검색 품질 집계 API 401, 회원 탈퇴 `DELETE` 401을 확인했다. Preview 로그에는 조회 시점 오류가 없었고 Production alias는 변경하지 않았다.
