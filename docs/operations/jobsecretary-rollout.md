...
- 2026-09-21 최신 로컬 커밋 Preview(`coverlettervault-13rwwchrt-junhas-projects-a748ef77.vercel.app`)가 `READY`가 되었고, 보호를 우회한 `vercel curl`로 `/` 200, 비로그인 `/career`·`/style` 307, 승인 활동 API·검색 품질 집계 API·회원 탈퇴 `DELETE` 401을 확인했다. 활동·자료 검색 필터와 공개 PDF URL 수집, 레거시 계정 정리 변경을 포함한 Preview에서 `vercel logs --level error --level warning`은 조회 시점 로그가 없었고 Production alias는 변경하지 않았다.
