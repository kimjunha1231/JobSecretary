...
4. 선택된 예문은 세션 상세와 개요·초안 생성 context에 동일하게 반영되고, 다른 문항의 문항별 예문은 기존 문항 범위를 넘지 않는다.
...
- 세션 상세 조회에서 저장된 선택을 다시 적용해 말투 context와 실제 생성 입력의 예문 집합이 어긋나지 않게 했다. 예문 원문은 기존 `style_examples`에서만 읽고 세션에는 복제하지 않는다.
